...
public class ModelCheckpointIndexRetentionTests extends AbstractADTest {
    ...
    public void testRunWithCleanupAsNeeded() throws Exception {
        doAnswer(invocation -> {
            ...
            return null;
        }).when(indexCleanup).deleteDocsBasedOnShardSize(eq(CommonName.LEGACY_OPENDISTRO_CHECKPOINT_INDEX_NAME), eq(50 * 1024 * 1024 * 1024L), any(), any());

        modelCheckpointIndexRetention.run();
        verify(indexCleanup, times(2))
            .deleteDocsBasedOnShardSize(eq(CommonName.LEGACY_OPENDISTRO_CHECKPOINT_INDEX_NAME), eq(50 * 1024 * 1024 * 1024L), any(), any());
        verify(indexCleanup).deleteDocsByQuery(eq(CommonName.LEGACY_OPENDISTRO_CHECKPOINT_INDEX_NAME), any(), any());
    }
    ...
    public void testRunWithCleanupAsFalse() throws Exception {
        ...
        modelCheckpointIndexRetention.run();
        verify(indexCleanup).deleteDocsBasedOnShardSize(eq(CommonName.LEGACY_OPENDISTRO_CHECKPOINT_INDEX_NAME), eq(50 * 1024 * 1024 * 1024L), any(), any());
        verify(indexCleanup).deleteDocsByQuery(eq(CommonName.LEGACY_OPENDISTRO_CHECKPOINT_INDEX_NAME), any(), any());
    }
}
